...
```
UryWu@MacBookPro MINGW64 /g/Projects/projects_Qt/telegram_explorer (master)
$ git log --graph --oneline --decorate --all
* 5f8b66a (HEAD -> master) fix: use correct FFmpeg 8.x DLL names with version nu
mbers
...
我想回到7fb5a29备份点，然后重新创建一个分支，然后原master分支重命名为ffmpeg-videoplayer分支，新分支为master分支。
```
...
```git
# 1. 首先，将当前的 master 分支重命名为 ffmpeg-videoplayer
git branch -m master ffmpeg-videoplayer

# 2. 基于 7fb5a29 创建新的 master 分支，并切换到新master分支
git checkout -b master 7fb5a29

# 3. 查看结果，确认分支结构
git log --graph --oneline --decorate --all --all
```
...
# 2. 基于 7fb5a29 直接创建新 master（不切换 HEAD）
...
你的提醒很重要 —— **保持 master 的线性发展**，从备份点重新开始，原来的开发分支保留作为特性分支。
